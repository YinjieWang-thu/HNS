...
G4double px_p, py_p, pz_p, E_p, px_K, py_K, pz_K, E_K, px_Sg, py_Sg, pz_Sg, E_Sg, w;
    while(infile >> px_p >> py_p >> pz_p >> E_p >> px_K >> py_K >> pz_K >> E_K >> px_Sg >> py_Sg >> pz_Sg >> E_Sg >> w ) {
        ParticleData data;
        G4ThreeVector mom_p(px_p* GeV, py_p* GeV, pz_p* GeV);
        G4ThreeVector mom_K(px_K* GeV, py_K* GeV, pz_K* GeV);
        G4ThreeVector mom_Sg(px_Sg* GeV, py_Sg* GeV, pz_Sg* GeV);
        data.momentum_p = mom_p;
        data.momentum_K = mom_K;
        data.momentum_Sg = mom_Sg;
        data.energy_p = (E_p-0.93827) * GeV;
        data.energy_K = (E_K-0.49368) * GeV;
        data.energy_Sg = (E_Sg-1.192642) * GeV;
        data.weight = w;
...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* event)
{
  ...
  G4ParticleDefinition* particle3 = particleTable->FindParticle(particleName = "sigma0");
  ...
  fParticleGun->SetParticleMomentum(pd.momentum_Sg);
  fParticleGun->SetParticleEnergy(pd.energy_Sg);
  fParticleGun->SetParticlePosition(G4ThreeVector(0, 0, 0));
